...
public class PetriNetView extends Observable implements Cloneable, IObserver, Serializable, Observer {
    ...
    public PetriNetView(ArrayList<PlaceView> _placeViews, ArrayList<TransitionView> _transitionViews,
            ArrayList<ArcView> _arcViews, ArrayList<InhibitorArcView> _inhibitorViews,
            ArrayList<AnnotationNote> _labels, ArrayList<RateParameter> _rateParameters,
            Vector<Vector<String>> functionRelatedPlaces, int _selectedTokenTypeNumber,
            LinkedList<MarkingView>[] _initialMarkingVector, LinkedList<MarkingView>[] _currentMarkingVector,
            int[] _capacityMatrix, int[] _priorityMatrix, boolean[] _timedMatrix,
            LinkedList<MarkingView>[] _markingVectorAnimationStorage, Hashtable _arcsMap,
            Hashtable _inhibitorsMap, ArrayList _stateGroups,
            PetriNetController _petriNetController, PetriNet _model,
            TokenSetController _tokenSetController) {
        this._placeViews = _placeViews;
        this._transitionViews = _transitionViews;
        this._arcViews = _arcViews;
        this._inhibitorViews = _inhibitorViews;
        this._labels = _labels;
        this._rateParameters = _rateParameters;
        this.functionRelatedPlaces = functionRelatedPlaces;
        this._selectedTokenTypeNumber = _selectedTokenTypeNumber;
        this._initialMarkingVector = _initialMarkingVector;
        this._currentMarkingVector = _currentMarkingVector;
        this._capacityMatrix = _capacityMatrix;
        this._priorityMatrix = _priorityMatrix;
        this._timedMatrix = _timedMatrix;
        this._markingVectorAnimationStorage = _markingVectorAnimationStorage;
        this._arcsMap = _arcsMap;
        this._inhibitorsMap = _inhibitorsMap;
        this._stateGroups = _stateGroups;
        this._petriNetController = _petriNetController;
        this._model = _model;
        this._tokenSetController = _tokenSetController;
    }
    ...
    public Hashtable getArcsMap() {
        return _arcsMap;
    }

    public Vector<Vector<String>> getFunctionRelatedPlaces() {
        return functionRelatedPlaces;
    }

    public ArrayList<InhibitorArcView> getInhibitorViews() {
        return _inhibitorViews;
    }

    public Hashtable getInhibitorsMap() {
        return _inhibitorsMap;
    }

    public ArrayList<AnnotationNote> getLabels() {
        return _labels;
    }

    public LinkedList<MarkingView>[] getMarkingVectorAnimationStorage() {
        return _markingVectorAnimationStorage;
    }
    ...
    public ArrayList<PlaceView> getPlaceViews() {
        return _placeViews;
    }

    public HashSet getRateParameterHashSet() {
        return _rateParameterHashSet;
    }

    public ArrayList<RateParameter> getRateParameters() {
        return _rateParameters;
    }
    ...
    public TokenSetController getTokenSetController() {
        return _tokenSetController;
    }
            ...
            for (int j = 0; j < placeCount; j++) {
                ...
                for (MarkingView m : markings[j]) {
                    if (m.getToken().isEnabled()) {
                        ...
                        if ((m.getCurrentMarking() < (m.getToken()).getBackwardsIncidenceMatrix().get(j, i)) && (m.getCurrentMarking() != -1)) {
                ...
                // capacities
                if (allTokenClassesEnabled && (_capacityMatrix[j] > 0) && (totalMarkings + totalForwardIncidenceMarkings - totalBackwardIncidenceMarkings
