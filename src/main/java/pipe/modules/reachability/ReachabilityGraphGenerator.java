...
/**
 * @author Matthew Worthington / Edwin Chung / Will Master /yazan Created module
 * to produce the reachability graph representation of a Petri net. This module
 * makes use of modifications that were made to the state space generator to
 * produce a list of possible states (both tangible and non tangible) which are
 * then transformed into a dot file. The file is then translated into its
 * graphical layout using www.research.att.com hosting of Graphviz. It should
 * therefore be noted that a live internet connection is required.
 * (Feb/March,2007)
 */
public class ReachabilityGraphGenerator
    ...
    private static final Checkbox checkBox2
            = new Checkbox("Show Deadlock", false);
    ...
    // our code for graduated proj
    public static File reachabilityGraphFile;
    public static PetriNetView DataLayer;
    public static DefaultGraph Graph;
...
//		if(pnmlData.getTokenViews().size() > 1)
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            if (!sourcePetriNetView.hasPlaceTransitionObjects()) {
                ...
                try {
                    ...
                    String place1 = temp.substring(0, Uindex);
                    String place2 = temp.substring(Uindex + 1, temp.length());
                    ...
                    boolean m = NextState(place1, place2, nodes);
                    for (Node node : nodes) {
    ...
    public boolean NextState(String place1, String place2, List<Node> nodes) {
        PetriNetView sourcePetriNetView = ApplicationSettings.getApplicationView().getCurrentPetriNetView();
        PlaceView p1 = sourcePetriNetView.getPlaceByName(place1);
        PlaceView p2 = sourcePetriNetView.getPlaceByName(place2);
        String foo = null;
        if (place1.equals(foo)) {
            int index = legend.indexOf(p2);
            int a[] = listMarkings.get(2);
            ...
            int index1 = legend.indexOf(place1);
            int index2 = legend.indexOf(place2);
            boolean check = false;
            int count = 0;
            for (int[] marking : listMarkings) {
                if (check == true) {
                    int a[] = listMarkings.get(count);
                    if (a[index2] > 0 || a[index2] == -1) {
                        return true;
                    } else {
                        check = false;
                    }
                } else if (check == false) {
                    if (marking[index1] > 0 || marking[index1] == -1) {
                        check = true;
                    }
                    count++;
                }
...
//                                  changed this to get currentMarkingVector
                    ...
                    if (checkBox2.getState()) {
                        findDeadlocks(reachabilityGraph, sourcePetriNetView,
                                generateCoverability);
                    } else {
    ...
    //test
    private void findDeadlocks(File rgFile, PetriNetView dataLayer,
        ...
        //  nodes.clear();
        //    nodes.add(dead);
        graph.addElements(deadlocks, null);
        frame.constructGraphFrame(graph, "Deadlock");
    }
